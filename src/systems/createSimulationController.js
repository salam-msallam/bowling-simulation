...
export function createSimulationController({
  ...
  function updateSimulationHUD() {
    // يحدث لوحة HUD بقيم الكرة والاحتكاك وعدد الإطارات الحالية.
    const ball = ballPhysics.getState()
  ...
  function launchBall(v0, angle, revRate, oilPattern = 'Medium') {
    // نقطة بدء الرمية من الواجهة أو زر Space.
    resetFrameState()
    currentOilPattern = oilPattern
    currentOilZone = OilZone.fromPreset(oilPattern)
